...
export default function FeedbackCard({ feedback, onEdit, onDelete }: FeedbackCardProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-3xl font-semibold text-[#111]">{t('feedbackCard.title')}</h1>
        <p className="mt-2 text-sm text-neutral-500">{t('feedbackCard.subtitle')}</p>
      </div>
    ...
    <span key={i} className="inline-flex transform scale-[1.5] origin-center">
      ...
      {feedback.comment ? (
        <div>
          ...
          <p className="mt-2 text-center text-xs text-neutral-400">{t('feedbackCard.helpsImprove')}</p>
        </div>
      ) : (
        <p className="text-center text-sm text-neutral-500">{t('feedbackCard.noComments')}</p>
      )}
      ...
      <div className="flex items-center justify-center gap-4">
        ...
        >
          <span className="inline-flex items-center gap-2"><TrashIcon className="h-4 w-4" /> {t('feedbackCard.delete')}</span>
        </button>
        <button
          type="button"
        ...
        >
          <span className="inline-flex items-center gap-2"><PencilSquareIcon className="h-4 w-4" /> {t('feedbackCard.edit')}</span>
        </button>
      </div>
    </div>
  );
