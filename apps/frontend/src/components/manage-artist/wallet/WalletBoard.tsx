...
import type { TransactionResponseDTO, WalletResponseDTO } from "@/types/transaction.dto";
...
interface WalletBoardProps {
    muaId: string; // required to fetch wallet & transactions
}
...
export const WalletBoard = ({ muaId }: WalletBoardProps) => {
    ...
    // CAPTURED section
    const [captured, setCaptured] = useState<SectionState>({ items: [], total: 0, page: 1, totalPages: 1, loading: false });
    // HOLD section
    const [hold, setHold] = useState<SectionState>({ items: [], total: 0, page: 1, totalPages: 1, loading: false });
  ...
  useEffect(() => {
    ...
    const loadTransactions = async (status: "CAPTURED" | "HOLD", page: number) => {
        ...
        try {
            const res = await TransactionService.getTransactionsByMuaId(muaId, { page, pageSize, status });
            ...
            <div className="rounded-xl border border-rose-200 bg-white shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md hover:-translate-y-[2px]">
                <div className="flex items-center justify-between p-5 border-b border-rose-100 bg-gradient-to-r from-rose-50 to-white">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-bold tracking-wide text-[#111]">Wallet Balance</h2>
                            <p className="text-xs text-rose-500">Available balance (CAPTURED)</p>
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="text-2xl font-black text-[#111]">
                            {loadingWallet ? <span className="animate-pulse text-rose-400">•••</span> : `${formatMoney(wallet?.balance || 0)} ₫`}
                        </div>
                        <div className="text-xs text-neutral-500">Currency: {wallet?.currency || "VND"}</div>
                    </div>
                </div>
                <div className="grid grid-cols-1 gap-4 p-5 md:grid-cols-3">
                    <div className="rounded-lg border border-neutral-200 p-4 bg-white transition-all duration-300 hover:-translate-y-[1px] hover:shadow-sm">
                        <div className="text-xs text-neutral-500">Captured count</div>
                        <div className="text-lg font-semibold text-[#111]">{captured.total}</div>
                    </div>
                    <div className="rounded-lg border border-neutral-200 p-4 bg-white transition-all duration-300 hover:-translate-y-[1px] hover:shadow-sm">
                        <div className="text-xs text-neutral-500">Hold count</div>
                        <div className="text-lg font-semibold text-[#111]">{hold.total}</div>
                    </div>
                    <div className="rounded-lg border border-neutral-200 p-4 bg-white transition-all duration-300 hover:-translate-y-[1px] hover:shadow-sm">
    ...
    MUA - {user.fullName}
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <TransactionList
                    ...
                    onPrev={() => loadTransactions("CAPTURED", Math.max(1, captured.page - 1))}
